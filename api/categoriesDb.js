...
function insert(category) {
    return db('categories').insert(category).returning('id').then(idArr => {
        const id = idArr[0];
...
function update(selector = null, payload) {
    ...
    return db('categories').where(selector).update(payload).returning('id').then(idArr => {
        const id = idArr[0];
        return db('categories').where({
            id
        }).first();
...
function remove(selector = null) {
    ...
    return db('categories').where(selector).delete().returning('id');
}
