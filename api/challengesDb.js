...
function insert(challenge) {
    return db('challenges').insert(challenge).returning('id').then(idArr => {
        const id = idArr[0];
        return db('challenges').where({
            id
        }).first();
    });
}
...
function update(selector = null, payload) {
    ...
    return db('challenges').where(selector).update(payload).returning('id').then(idArr => {
        const id = idArr[0];
        return db('challenges').where({
            id
        }).first();
...
function getMany(filter = {}) {
    return db('challenges').where(filter);
}
...
function getOne(filter = null) {
    ...
    return db('challenges').where(filter).first();
}